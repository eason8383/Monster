...
@property (nonatomic, strong) SliderMenuView *contentView;
...
- (instancetype)initWithSliderMenu{
    if (self = [super init]) {
        _isSingleSelect = NO;
...
//    blurImage = [blurImage applyBlurWithRadius:5 tintColor:self.tintColor saturationDeltaFactor:1.8 maskImage:nil];
    ...
    [self.view insertSubview:self.blurView belowSubview:self.contentView];//插入頁面
...
- (void)tapDnBtn:(UIButton*)button{
    if ([self.delegate respondsToSelector:@selector(sidebar:didTapItemAtIndex:)]) {
        [self.delegate sidebar:self didTapItemAtIndex:button.tag];
    }
}
...
//    self.contentView.contentSize = CGSizeMake(270, kScreenHeight);
