...
#define scanContent_Y self.frame.size.height * 0.28
/** 扫描内容的X值 */
...
//    [layer addSublayer:right_imageView_down.layer];
    ...
    CGFloat promptLabelY = CGRectGetMinY(scanContent_layer.frame) - 71;
    ...
    promptLabel.text = @"对准二维码或条码，即可自动扫描";
...
//    UIImage *lineimg = [UIImage imageNamed:@"SGQRCode.bundle/QRCodeScanningLine"];
    UIImage *lineimg = [UIImage imageNamed:@"line_scan-1"];
    lineImgView = [[UIImageView alloc] init];
    lineImgView.image = lineimg;
//    CGFloat lineimgViewX = 0;
//    CGFloat lineimgViewY = scanContent_Y;
//    CGFloat lineimgViewW = self.frame.size.width;
//    CGFloat lineimgViewH = lineimg.size.height;
    CGFloat lineimgViewX = left_imageView.frame.origin.x;;
    CGFloat lineimgViewY = CGRectGetMinY(scanContent_layer.frame) - left_image.size.width * 0.5 + margin;
    CGFloat lineimgViewW = right_imageView.frame.origin.x - left_imageView.frame.origin.x + right_imageView.frame.size.width;
    CGFloat lineimgViewH = lineimg.size.height;

    lineImgView.frame = CGRectMake(lineimgViewX, lineimgViewY, lineimgViewW, lineimgViewH);
    [self addSubview:lineImgView ];
    [self insertSubview:lineImgView belowSubview:left_imageView];
    
    // line移动的范围为 一个扫码框的高度(由于图片问题再减去图片的高度)
//    lineAnimation = [self animationWith:@(0) toValue:@(scanContent_layerH - lineimgViewH) repCount:MAXFLOAT duration:1.5f];
    lineAnimation = [self animationWith:@(- lineimgViewH) toValue:@(scanContent_layerH - lineimgViewH) repCount:MAXFLOAT duration:1.5f];
